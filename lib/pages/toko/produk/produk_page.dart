...
class _ProdukPageState extends State<ProdukPage> {
  ...
  Future<void> _detailProdukList(int product_id) async {
    debugPrint("DEBUG: Produk ID yang dikirim ke detailProduk: $product_id");
    ...
    if (_userLoggedIn) {
      ...
      if (status.isSuccess) {
        debugPrint("DEBUG: Detail produk berhasil diambil untuk ID: $product_id");
        Get.to(() => UbahProdukPage());
      } else {
        debugPrint("DEBUG: Gagal mengambil detail produk untuk ID: $product_id, pesan: ${status.message}");
        AwesomeSnackbarButton("Gagal", status.message, ContentType.failure);
      }
    } else {
      debugPrint("DEBUG: User belum login, tidak bisa buka detail produk.");
    }
  }
          ...
          children: [
            ...
            GetBuilder<PopularProdukController>(
              builder: (controller) {
                ...
                if (controller.daftarProdukList.isEmpty) {
                  return Column(
